...
void TrtRunner::set_input_shape(const std::string& name, const std::vector<int64_t>& shape) {
    nvinfer1::Dims dims{};
...
void TrtRunner::copy_input(const std::string& name, const float* host_ptr, size_t count) {
...
void TrtRunner::prepare_output(const std::string& name, size_t count) {
    bind_tensor(name, count * sizeof(float));
    auto& info = buffers_[name];
    if (!context_->setTensorAddress(name.c_str(), info.device_ptr)) {
...
void TrtRunner::copy_output(const std::string& name, float* host_ptr, size_t count) {
...
size_t TrtRunner::get_volume(const std::string& name) const {
...
void TrtRunner::bind_tensor(const std::string& name, size_t bytes) {
    auto& info = buffers_[name];
    ...
    if (cudaMalloc(&info.device_ptr, bytes) != cudaSuccess) {
        throw std::runtime_error("cudaMalloc failed for tensor " + name);
    }
    info.bytes = bytes;
}
